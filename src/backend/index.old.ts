...
import { testCreateUser } from './api/testCreateUser.ts';
  ...
  async fetch(
    ...
    try {
      ...
      if(path.startsWith('/api/test')){
        //
        const result =await testCreateUser(request, env)
      ...
      if (path.startsWith('/api/')) {
        try {
          ...
          console.log({session})
          // const authenticatedRequest = await authenticateRequest(request, authRequest);
